...
def read_data():
    ...
    df = pd.read_csv(data_file, names=['x', 'y'], parse_dates=['x'], skiprows=idx)
    idx += len(df)
    data_file.close()
...
p = figure(title="RF Class Registrator", x_axis_label='time', x_axis_type='datetime',  y_axis_label='# of students')
r = p.line(x='x', y='y', source=ds, line_width=2)
